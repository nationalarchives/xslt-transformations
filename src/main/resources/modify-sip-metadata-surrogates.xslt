...
<xsl:stylesheet version="2.0" xmlns="http://www.tessella.com/XIP/v4"
                ...
                exclude-result-prefixes="ext xip xs fntc xsi">
    ...
    <xsl:param name="born-digital" select="'false'"/> <!-- wouldn't this always be false for surrogates? -->
    ...
    <xsl:template match="/XIP">
        <xsl:copy>
            ...
            <DeliverableUnits>
                <xsl:call-template name="create-metadata-du">
                ...
                <xsl:if test="$born-digital != 'true'">
                    <xsl:for-each select="/XIP/Files/File[WorkingPath = '/']">
                        ...
                        <xsl:variable name="from" select="concat($contentLocation, '/', FileName)"/>
                        <xsl:variable name="to"
                            select="concat($contentLocation, '/_metadata/', $unit, '/', $series, '/', FileName)"/>
                        <xsl:message
                ...
                <xsl:message select="concat('XSLT FileSystem: Renaming ''', $from, ''' to ''', $to, '''')"/>
                <xsl:choose>
                ...
                <xsl:if test="$born-digital != 'true'">
                    <xsl:for-each select="/XIP/Files/File[WorkingPath = '/']">
                        ...
                        <xsl:call-template name="create-manifestation">
                            ...
                            <xsl:with-param name="file-path" select="string-join(('_metadata', $unit, $series),'/')"/>
                        </xsl:call-template>
                    </xsl:for-each>
                </xsl:if>

                <!-- reproduce all existing manifestations apart from the 'root' one -->
                <xsl:apply-templates select="/XIP/DeliverableUnits/Manifestation[DeliverableUnitRef != $root-uuid]"/>
            </DeliverableUnits>
    ...
    <xsl:template match="ParentRef[.=$root-uuid and ../Title !='content']">
        ...
        <xsl:variable name="to" select="concat($contentLocation, '/_metadata/', $unit, '/', $series, '/', ../Title)"/>
        <xsl:message select="concat('XSLT FileSystem: Moving from ''', $from, ''' to ''', $to, '''')"/>
        <xsl:sequence select="ext:move-file($from, $to)"/>
    </xsl:template>
    ...
    <xsl:template match="SecurityTag[../ParentRef=$root-uuid and ../Title = 'content']">
        <xsl:copy-of select="."/>
        <xsl:apply-templates select="$root-du/Metadata"/>
        <!--<xsl:copy-of select="$root-du/Metadata"/>-->
    </xsl:template>
    
    <!-- content du metadata must lose any dc:relation parent reference -->
    <!-- <xsl:template
        match="tnaxm:metadata/dc:relation[../../../ParentRef=$root-uuid and ../../../Title ='content']"/>

    <xsl:template match="tnaxm:metadata/dc:title[. eq 'content']">
        <dc:title>
            <xsl:value-of select="$displayable-series"/>
        </dc:title>
    </xsl:template>-->
    ...
    <xsl:template match="tna:metadata/rdf:RDF//tna:digitalFile/tna:DigitalFile/tna:filePathAndName">
        <xsl:variable name="old-path">
            <xsl:value-of select="."/>
        </xsl:variable>
        <xsl:variable name="new-path">
            <xsl:value-of select="replace(replace($old-path, '/content', ''),$series,encode-for-uri($displayable-series))"/>
        </xsl:variable>
  ...
  -->
    ...
    <xsl:template name="create-metadata-du">
        <xsl:param name="uuid"/>
        ...
        <DeliverableUnit status="new">
            ...
            <DigitalSurrogate>false</DigitalSurrogate> <!-- metadata DUs are never surrogates -->
            <CatalogueReference>
